...
<script type="application/javascript">
    ...
    langS['Ru'] = {
        ...
        GameName: "УБЕЙ КРОТОВ",
        StudiosName: "NULAY GAME",
        YourLoose: "Вы проиграли!!!",
    ...
    langS['En'] = {
        ...
        Lang: "Langvich",
        GoToMenu: "Go to menu",
        GameName: "KILLER MOLES",
        StudiosName: "NULAY GAME",
        YourLoose: "Your LOOSE!!!",
        YouScore: "You score ",
        From: " from ",
        Moles: "moles",
        LangName: "English",
    ...
    Krot.prototype = {
        ...
        initialize: function (option) {
            ...
            this.funcM = [];
            this.mus = [];
            var poleG = $('#poleGame').css({
                'position': 'relative',
                'width': this.size[0] + 'px',
                'height': this.size[1] + 'px',
                'font-size': Math.round(this.size[0] / 800) + 'em'
            });
            poleG.append($('<div style="position:absolute;top:0;left:0;z-index:20;">').append($('<img src="' + this.imgFolder + 'pole.png">').css({
                'width': this.size[0] + 'px',
                'height': this.size[1] + 'px'
            })));
            poleG.append($('<div style="position:absolute;top:0;left:0;z-index:10;">').append($('<img src="' + this.imgFolder + 'pole2.png">').css({
                'width': this.size[0] + 'px',
                'height': this.size[1] + 'px'
            })));
            poleG.append($('<div style="position:absolute;top:0;left:0;z-index:25;">').css({
                'position': 'relative',
                'width': this.size[0] + 'px',
                'height': this.size[1] + 'px'
            }));
            ...
            this.action = 'keydown';
            for (var i = 0; i < 4; i++) {
                this.krotCl[i] = $('<div style="position:absolute;top:' + (this.axis[i].t - 40) + 'px;left:' + this.axis[i].l + 'px;z-index:30;">').append($('<img src="' + this.imgFolder + 'krot1.png">').css({
                    'visibility': 'hidden',
                    'width': Math.round(57 * this.scaleW) + 'px',
                    'height': Math.round(58 * this.scaleH) + 'px'
                }));
                this.krot[i] = $('<div style="position:absolute;top:' + this.axis[i].t + 'px;left:' + this.axis[i].l + 'px;z-index:15;">').append($('<img src="' + this.imgFolder + 'krot1.png">').css({
                    'width': Math.round(57 * this.scaleW) + 'px',
                    'height': Math.round(58 * this.scaleH) + 'px'
                }));
                this.func[i] = this.getFunc(i, 0);
                this.funcM[i] = this.getFunc(i, 2);
                this.bit[i] = $('<img src="' + this.imgFolder + 'mozgcrack.gif?ind=' + i + '"/>').css({
                    'width': Math.round(57 * this.scaleW) + 'px',
                    'height': Math.round(58 * this.scaleH) + 'px'
                }).hide();
        ...
        setAxisAndKey: function () {
            ...
            this.axis[1] = {
                't': Math.round(this.size[1] * 0.64),
                'l': Math.round(this.size[0] * 0.78),
                'kC': keyVal[1]
            };
            this.axis[2] = {
                't': Math.round(this.size[1] * 0.83),
                'l': Math.round(this.size[0] * 0.51),
                'kC': keyVal[2]
            };
        ...
        getFunc: function (ind, numf) {
            ...
            switch (numf) {
                case 0:
                    return function (ev) {
                        var keyK = (ev.keyCode) ? ev.keyCode : ev.which;
                        if (keyK == thisEl.axis[ind].kC) {
                            thisEl.actionDo(ind, ev);
                        }
                    };
                    break;
                case 1:
                    return function () {
                        thisEl.krotTudaSuda(ind);
                    };
                    break;
                case 2:
                    return function (ev) {
                        ...
                        thisEl.krot[ind].animate({top: "+=" + thisEl.goTop}, thisEl.timeUpDown,
                            function () {
                                ...
                                if (thisEl.count - thisEl.score > 4) {
                                    ...
                                    setTimeout(function () {
                                        thisEl.krotTudaSuda(ind);
                                    }, thisEl.getRandomInt(thisEl.minTimeNotShow, thisEl.maxTimeNotShow));
                                }
                            });
        ...
        endGame: function () {
            if (this.gameStart == true) {
                ...
                var bb = $('<div style="position:absolute;top:200px;left:0;z-index:50;">').css({
                    'width': this.size[0] + 'px',
                    'height': (this.size[1] - 300) + 'px'
                }).append('<div style="font-size:50px;text-align:center;">' + this.lang.YourLoose + '</div>').append('<div style="font-size:30px;text-align:center;"> ' + this.lang.YouScore + this.score + this.lang.From + this.count + '</div>').append(kd);
                kd.click(function () {
                    bb.remove();
                    mainMenu.menuShow();
                });
